...
export default class Graphics2d {
  ...
  */
  constructor(container, width, height) {
    ...
    /*
    const TEXTURE_IMAGE_FILE_NAME = 'images/tex_brain_00.jpg';

    const textureName = TEXTURE_IMAGE_FILE_NAME;
    const loader = new THREE.TextureLoader();
    loader.load(textureName, (texMap) => {
      // this.m_texMap = texMap;
      const wImg = texMap.image.width;
      const hImg = texMap.image.height;
      // console.log(`Loaded texture dim = ${wImg} * ${hImg}`);
      texMap.minFilter = THREE.LinearFilter;
      texMap.magFilter = THREE.LinearFilter;
      matTex2d.create(texMap, (mat) => {
        this.m_material = mat;
        this.m_mesh = new THREE.Mesh(this.m_geometry, this.m_material);
        this.m_scene.add(this.m_mesh);
        // console.log('Create 2d material end. Scene added');
      });
    });
    */
  ...
  */
  ...
  /**
  * Create internal geo structures for 2d slises visualization
  */
  createTileMaps() {
    if ((this.m_volumeData === null) || (this.m_volumeHeader === null)) {
      return;
    ...
    if (this.m_volumeHeader.m_glFormat !== KtxHeader.KTX_GL_RED) {
      console.log('Not 1 byte volume texture. Need to implement');
      return;
    }
