...
export default class AngleTool {
  ...
  updateLines(zoom, posX, posY) {
    ...
    for (let i = 0; i < length; ++i) {
      ...
      this.m_angles[i].text.updateText(
        x1,
        y1,
        this.m_textWidthScr,
  ...
  onMouseMove(x, y) {
    ...
    switch (this.m_state) {
      ...
      case angleToolState.FIRST_LINE: {
        ...
        this.m_scene.remove(angle.line1.getRenderObject());
        const line1 = new Line2D(this.m_scene, this.m_lineWidth, this.m_xStart, this.m_yStart, x, y, this.m_linesMaterial);
      ...
      case angleToolState.SECOND_LINE: {
        const len1 = Math.sqrt(this.m_firstVector.x * this.m_firstVector.x + this.m_firstVector.y * this.m_firstVector.y);
        ...
        angle = this.m_angles.pop();
        this.m_scene.remove(angle.line2.getRenderObject());
        const line2 = new Line2D(this.m_scene, this.m_lineWidth, this.m_xStart, this.m_yStart, x, y, this.m_linesMaterial);
        this.m_scene.remove(angle.text);
        const strMsg = `${(
          (Math.acos(
            (this.m_firstVector.x * (x - this.m_xStart) +
