  ...
  */
  readFromFiles(files, callbackComplete, callbackProgress) {
    ...
    if (numFiles === NUM_FILES_VOL_SINGLE) {
      ...
      const fileHdrInt = files[0];
      const fileImgInt = files[1];
      const fileHdrRoi = files[2];
      const fileImgRoi = files[3];
      if (fileHdrInt.name.indexOf('.hdr') === -1) {
        console.log('1st file should have hdr extension');
      }
      if (fileImgInt.name.indexOf('.img') === -1) {
        console.log('2nd file should have img extension');
      }
      if (fileHdrRoi.name.indexOf('.hdr') === -1) {
        console.log('3rd file should have hdr extension');
      }
      if (fileImgRoi.name.indexOf('.img') === -1) {
        console.log('4th file should have img extension');
      }
