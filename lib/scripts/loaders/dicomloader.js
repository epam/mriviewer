...
/**
* Dicom folder loader
* @module app/scripts/loaders/dicomloader
*/
...
const NEED_SCAN_EMPTY_BORDER = true; // false;
...
export default class DicomFolderLoader {
  ...
  */
  ...
  createVolumeFromSlices() {
    ...
    VolumeTools.buildSmoothedHistogram(histogram, histSmooothed, maxVal, HIST_SMOOTH_SIGMA);
    ...
    const scale = (1 << (BITS_IN_BYTE + BITS_ACCUR)) / maxVal;
    ...
    loader.readFile((fileArrBu) => {
      ...
      if ((status !== LoadResult.SUCCESS) && (this.m_numFailsLoad === 0)) {
        ...
        if (this.m_callbackRead !== null) {
          this.m_callbackRead(status, null, 0, null);
          return;
        }
      }
  ...
  */
  readFiles(files, callbackRead, callbackProgress) {
    ...
    for (let i = 0; (i < this.m_numLoadedFiles) && (this.m_numFailsLoad < 1); i++) {
      ...
      this.runLoader(file.name, loader, i, callbackProgress);
    }
  }
  ...
  */
  readReadyFileList(arrBuf) {
    ...
    for (let i = 0; (i < this.m_numLoadedFiles) && (this.m_numFailsLoad < 1); i++) {
      ...
      const loader = this.m_loaders[i];
      this.runLoader(arrFileNames[i], loader, i, callbackProgress);
  ...
  */
  parseDicomFileBuffer(indexFile, arrayBuf) {
    ...
    const BITS_IN_BYTE = 8;
    const imageSizeBytes = this.m_xDim * this.m_yDim * (this.m_bitsPerPixel / BITS_IN_BYTE);
    ...
    if (this.m_bitsPerPixel === BITS_16) {
      ...
      if ((pixelMaxValue === -1) || (pixValDif === 0)) {
        ...
        const valScale = (MAX_SCALED_VAL << SCALE_BIT_ACC) / (pixelMaxValue - pixelMinValue);
        for (i = 0; i < numPixels; i++) {
