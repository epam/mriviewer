...
/**
* Nifti file loader
* @module app/scripts/loaders/roipalette
*/
  ...
  */
  createBytePalette256() {
    ...
    for (i = 0; i < numPalColors; i++) {
      ...
      const bCol = Math.floor(parseFloat(arrColor[OFFS_0]) * MAX_PAL_COLOR);
      const gCol = Math.floor(parseFloat(arrColor[OFFS_1]) * MAX_PAL_COLOR);
      const rCol = Math.floor(parseFloat(arrColor[OFFS_2]) * MAX_PAL_COLOR);
      const ind = parseInt(strIndexPalette, 10) * BYTES_PER_COLOR;
      this.m_palette256[ind + OFFS_0] = bCol;
      this.m_palette256[ind + OFFS_1] = gCol;
      this.m_palette256[ind + OFFS_2] = rCol;
    }
  }
  /**
