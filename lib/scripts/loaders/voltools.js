...
export default class VolumeTools {
  ...
  */
  static makeTextureSizeEven(pixelsSrc, xDim, yDim, zDim) {
    ...
    const zDimNew = zDim + 1;
    const numPixelsOld = xDim * yDim * zDim;
    const numPixelsNew = xDim * yDim * zDimNew;
    const pixelsDst = new Uint8Array(numPixelsNew);

    let i;
    for (i = 0; i < numPixelsOld; i++) {
      pixelsDst[i] = pixelsSrc[i];
    }
    for (; i < numPixelsNew; i++) {
      pixelsDst[i] = 0;
    }
    console.log(`modified texture size is: ${xDim} * ${yDim} * ${zDimNew}`);
    return pixelsDst;
  }
} // class VolumeTools
