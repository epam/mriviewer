...
export default class VolumeTools {
  ...
  */
  static makeTextureSizePowerOfTwoXY(pixelsSrc, xDimSrc, yDimSrc, zDimSrc, xDimDst, yDimDst) {
    ...
    const numPixelsOld = xDimSrc * yDimSrc * zDimSrc;
    if (pixelsSrc.length !== numPixelsOld) {
      console.log(`Error source volume bpp:  = ${xDimSrc} * ${yDimSrc} * ${zDimSrc}`);
    }
    const numPixelsNew = xDimDst * yDimDst * zDimDst;
    const pixelsDst = new Uint8Array(numPixelsNew);
    let i;
    for (i = 0; i < numPixelsNew; i++) {
      pixelsDst[i] = 0;
    ...
    for (let z = 0; z < zDimSrc; z++) {
      // const MASK_CONVERT_PROGRESS = 31;
      // if ((z & MASK_CONVERT_PROGRESS) === 0) {
      //   console.log(`progress convert z = ${z}`);
      // }
    ...
    return pixelsDst;
  } // end of makeTextureSizePowerOfTwoXY
  /**
