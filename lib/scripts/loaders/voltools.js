...
const VolToolsErrors = {
  VOLTOOLS_ERROR_OK: 1,
  VOLTOOLS_ERROR_BAD_MULTIPLIER: -1,
  VOLTOOLS_ERROR_BAD_DIMENSION: -2,
  VOLTOOLS_ERROR_BAD_ARRAY: -3,
  VOLTOOLS_ERROR_BAD_NUMBER: -4,
};
...
class VolumeTools {
  ...
  *
  * main formula: sharpened = blurred + (original - blurred) * multiplier
  *
  ...
  */
  static contrastEnchanceUnsharpMask(pixelsSrc,
    ...
    if ((typeof xDim !== 'number') || (typeof yDim !== 'number') || (typeof zDim !== 'number')) {
      return VolToolsErrors.VOLTOOLS_ERROR_BAD_NUMBER;
    }
    if (typeof pixelsSrc !== 'object') {
      return VolToolsErrors.VOLTOOLS_ERROR_BAD_ARRAY;
    }
    if (typeof pixelsDst !== 'object') {
      return VolToolsErrors.VOLTOOLS_ERROR_BAD_ARRAY;
    }
    if (multiplier < 1.0) {
      return VolToolsErrors.VOLTOOLS_ERROR_BAD_MULTIPLIER;
    }
    ...
    if ((xDim <= 1) || (yDim <= 1) || (zDim <= 1)) {
      return VolToolsErrors.VOLTOOLS_ERROR_BAD_DIMENSION;
    }
    ...
    for (let zc = 0; zc < zDim; zc++) {
      for (let yc = 0; yc < yDim; yc++) {
        for (let xc = 0; xc < xDim; xc++) {
          ...
          let  valSharpened = valSmoothed + (val - valSmoothed) * multiplier;
    ...
    return VolToolsErrors.VOLTOOLS_ERROR_OK;
  }
} // class VolumeTools

export { VolumeTools, VolToolsErrors };
