...
* @param {array} pixelsSrc - array of ARGB pixels
  */
  static showTexture2d(xDim, yDim, pixelsSrc) {
    const arrSize = pixelsSrc.length;
    const BYTES_IN_DWORD = 4;
    if (arrSize !== xDim * yDim * BYTES_IN_DWORD) {
      console.log(`!!! Bad texture size = ${arrSize}, but expected = ${xDim} * ${yDim}`);
      return -1;
    }
    ...
    canvas.width = xDim;
    canvas.height = yDim;
    const img = ctx.createImageData(xDim, yDim);
    // idata.data.set(pixelsSrc);
    for (let i = 0; i < img.data.length; i++) {
      img.data[i] = pixelsSrc[i];
    }
    // put on screen
    ctx.putImageData(img, 0, 0);
    document.body.appendChild(canvas);
    console.log(`!!! Displayed image data = ${xDim} * ${yDim}`);
    return 1;
  } // showTexture2d
  ...
  static extract2dSliceFrom3dTexture(xDim, yDim, zDim, pixelsSrc, sliceType, sliceIndex, pixelsDst) {
    ...
    if (sliceType === Z_SLICE) {
      ...
      for (y = 0; y < h; y++) {
        for (x = 0; x < w; x++) {
          const off = z * xDim *  yDim + y * xDim + x;
          ...
          pixelsDst[j + OFF_3] = pixelsSrc[off4 + OFF_3];
          j += BYTES_IN_DWORD;
        }
      }
    } // if z slice
