...
export default class ActiveVolume {
  ...
  */
  ...
  save() {
    ...
    // console.log('ActiveVolume.save()...');
    for (let i = 0; i < numPixels; i++) {
      this.m_pixelsSrc[i] = volTexDst[i];
      // this.m_pixelsSrc[i] = 0;
    }
    volTexDst = null;
  }
  ...
  */
  createGeoSphere() {
    ...
    const NUM_1 = 1;
    const NUM_2 = 2;
    let dimMin = (this.m_xWorldBox < this.m_yWorldBox) ? this.m_xWorldBox : this.m_yWorldBox;
    dimMin = (this.m_zWorldBox < dimMin) ? this.m_zWorldBox : dimMin;
    let xScale, yScale, zScale;
    if (dimMin > 0.0) {
      xScale = this.m_sphereRadius.x * this.m_xWorldBox / dimMin;
      yScale = this.m_sphereRadius.y * this.m_yWorldBox / dimMin;
      zScale = this.m_sphereRadius.z * this.m_zWorldBox / dimMin;
    } else {
      xScale = this.m_sphereRadius.x;
      yScale = this.m_sphereRadius.y;
      zScale = this.m_sphereRadius.z;
    }
    for (let i = 0, i4 = 0; i < numVertices; i++, i4 += COORDS_IN_VERTEX) {
      vertices[i4 + NUM_0] = this.m_sphereCenter.x + vertices[i4 + NUM_0] * xScale;
      vertices[i4 + NUM_1] = this.m_sphereCenter.y + vertices[i4 + NUM_1] * yScale;
      vertices[i4 + NUM_2] = this.m_sphereCenter.z + vertices[i4 + NUM_2] * zScale;
    } // for (i) all vertices
