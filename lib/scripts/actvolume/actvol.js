...
export default class ActiveVolume {
  ...
  */
  ...
  sphereEvolve(xDim, yDim, zDim, volTexSrc, volTexDst, geoSphere, geoWireSphere, valBarrier01) {
    ...
    console.log(`sphereEvolve: num vertices in pshere geo = ${numVertices}`);
    ...
    const attrWireVertices = geoWireSphere.getAttribute('position');
    ...
    for (let i = 0; i < numVertices; i++) {
      const v = new THREE.Vector3(attrVertices.getX(i),
        attrVertices.getY(i),
        attrVertices.getZ(i));
      v.normalize();
      ...
      for (j = J_END; (j >= J_START) && !hasFoundMinBorder; j--) {
        x = xc + Math.floor(v.x * j);
        y = yc + Math.floor(v.y * j);
        z = zc + Math.floor(v.z * j);
        // skip if outside of volume
        if ((x < 0) || (y < 0) || (z < 0) || (x >= xDim) || (y >= yDim) || (x >= zDim)) {
          continue;
        }
      ...
      let j;
      let x, y, z;
      for (j = J_START; j < MAX_HALF_DIM; j++) {
        x = xc + Math.floor(v.x * j);
        y = yc + Math.floor(v.y * j);
        z = zc + Math.floor(v.z * j);
        if ((x < 0) || (y < 0) || (z < 0) || (x >= xDim) || (y >= yDim) || (x >= zDim)) {
          // console.log(`Reach end: ${x},${y},${z}`);
          break;
        }
        const off = x + (y * xDim) + (z * xyDim);
        const val = volTexSrc[off];
        if (val < VAL_BORDER) {
          // console.log(`Found edge: ${x},${y},${z}`);
          break;
        }
      }
      // place result point in [-0.5 .. +0.5]
    ...
    geoWireSphere.getAttribute('position').needsUpdate = true;
