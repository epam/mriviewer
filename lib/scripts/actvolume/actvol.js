...
export default class ActiveVolume {
  ...
  */
  ...
  sphereEvolve(xDim, yDim, zDim,
    ...
    //const START_SCAN_RATIO = 0.2;
    //const NUM_2 = 2.0;
    //const DIAG_ONE_TRI = Math.sqrt(NUM_2);
    //const J_START = Math.floor(MAX_HALF_DIM * START_SCAN_RATIO);
    //const J_END = Math.floor(MAX_HALF_DIM * DIAG_ONE_TRI);
    ...
    const VAL_MIN_BORDER =  Math.floor(valBarrier01 * NUM_COLORS);

    // const SOME_ADD = 8;
    // const NUM_255 = 255;
    // let VAL_MAX_BORDER = Math.floor(VAL_MIN_BORDER + SOME_ADD);
    // if (VAL_MAX_BORDER > NUM_255) {
    //   VAL_MAX_BORDER = NUM_255;
    // }
    // console.log(`Autodetect brain shape with barrier range [ ${VAL_MIN_BORDER} , ${VAL_MAX_BORDER} ]`);
    console.log(`Autodetect brain shape with barrier range ${VAL_MIN_BORDER}`);


    // const vBoxMax = geoSphere.boundingBox.max;
    // const vBoxMin = geoSphere.boundingBox.min;
    // console.log(`GeoBoxMin: ${vBoxMin.x},${vBoxMin.y},${vBoxMin.z}`);
    // console.log(`GeoBoxMax: ${vBoxMax.x},${vBoxMax.y},${vBoxMax.z}`);
    ...
    for (let i = 0; i < numVertices; i++) {
      ...
      if (modeEvolve === ActiveVolume.SPHERE_EVOLVE_FROM_INSIDE) {
        ...
        const xVol = Math.floor((vSrc.x + HALF) * xDim);
        const yVol = Math.floor((vSrc.y + HALF) * yDim);
        const zVol = Math.floor((vSrc.z + HALF) * zDim);
        ...
        for (j = ONE; j < 2 * J_END; j++) {
          x = xVol + Math.floor(0.5*vn.x * j);
          y = yVol + Math.floor(0.5*vn.y * j);
          z = zVol + Math.floor(0.5*vn.z * j);
        /*const ONE = 1;
        // for (j = J_START; j < J_END; j++) {
        for (j = ONE; j < J_END; j++) {
          x = xVol + Math.floor(vn.x * j);
          y = yVol + Math.floor(vn.y * j);
          z = zVol + Math.floor(vn.z * j);
         */
          if ((x < 0) || (y < 0) || (z < 0) || (x >= xDim) || (y >= yDim) || (z >= zDim)) {
        ...
        // if (hasFoundVolExit) {
        //   console.log(`sphereEvolve. Stop at exit [${i}] = ${x}, ${y}, ${z}, `);
        // }
        // if (!hasFoundLowPixel) {
        //   console.log(`sphereEvolve. Min border (${VAL_MIN_BORDER}) not found for ${i} / ${numVertices} point`);
        // }
        // if (hasFoundLowPixel) {
        //   console.log(`sphereEvolve. Yes border[${i}] = ${x}, ${y}, ${z}, `);
        // }
