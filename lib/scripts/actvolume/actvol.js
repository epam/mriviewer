...
export default class ActiveVolume {
  ...
  */
  skullRemove(xDim, yDim, zDim, volTexSrc, volTexDst, needCreateMask, needLog) {
    ...
    // !!!!!!!! TEST !!!!!!!!
    // const TEST_NUM_STEPS_FOR_DEEP_DERBUG = 20;
    // numPredSteps = TEST_NUM_STEPS_FOR_DEEP_DERBUG;
    ...
    const resClip = VolumeClipper.clipVolumeByNonConvexGeo(this.m_pixelsSrc, this.m_xDim, this.m_yDim, this.m_zDim,
      volTexDst, geoRender, needCreateMask);
    if (resClip < 0) {
    ...
    if (NEED_SAVE_CLIPPED_VOLUME_KTX) {
      ...
      const TEST_CLIPPED_VOL_KTX_NAME = 'clipped.ktx';
