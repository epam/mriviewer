...
export default class ActiveVolume {
  ...
  */
  constructor() {
    ...
    this.m_isThisFirstLaplasianUpdate = 2;
  }
  ...
  */
  updateGeoNormalsUniformityColors(geo, normalSpeed) {
    ...
    this.m_isThisFirstLaplasianUpdate--;
    if (this.m_isThisFirstLaplasianUpdate === 0) {
      console.log(`Save geo after first laplasian update. NV=${numVertices},NT=${numTriangles},Sp=${normalSpeed}`);
      // console.log(`updateGeo.V[0]=${vertices[0]},NV[0]=${normals[0].x}`);

      this.m_isThisFirstLaplasianUpdate = 0;
      const FILE_NAME_GEO = 'lap_geo.obj';
    ...
    for (i = 0, i3 = 0, i4 = 0; i < numVertices; i++, i3 += NUM_COMPS_NORMAL, i4 += NUM_COMPS_VERTEX) {
      const v = new THREE.Vector3(vertices[i4 + OFF_0], vertices[i4 + OFF_1], vertices[i4 + OFF_2]);
      const vn = new THREE.Vector3(normals[i3 + OFF_0], normals[i3 + OFF_1], normals[i3 + OFF_2]);
      ...
      */
      ...
      const NEXT_STEP = 2.5;
      const nx = Math.floor(v.x + vn.x * NEXT_STEP);
      const ny = Math.floor(v.y + vn.y * NEXT_STEP);
      const nz = Math.floor(v.z + vn.z * NEXT_STEP);
      /*
      #ifdef _DEBUG
        assert(nx < m_xDim);
        assert(ny < m_yDim);
    ...
    const VERICES_UNIFORMITY = 0.46;
    if (aveUni < VERICES_UNIFORMITY) {
      return true;
    }
    return false;
  }
