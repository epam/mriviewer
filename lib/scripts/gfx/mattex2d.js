  ...
  constructor() {
    ...
    this.m_strShaderFragment = `
      ...
      void main() {
        ...
        if (showAll) {
          float vColTex = texture2D(texture, vecUV).a;
          float intensity = texture2D(texture, vecUV).a;
...
/*          if (vColTex.a > 0.4)
            ...
            col = vec4(2.0 * vColTex.a, 2.0 * vColTex.a, 2.0 * vColTex.a, 1.0);
*/          
            col = vec4(vColTex.a, vColTex.a, vColTex.a, 1.0);
          gl_FragColor = col;
          return;
        }
          ...
          float val = texture2D(texture, texCoord).a;
          float BIFICOBR = 0.0;
        ...
        for (float i = -2.0; i < 2.5; i += 1.0)
          for (float j = -2.0; j < 2.5; j += 1.0)
            for(float k = -2.0; k < 2.5; k += 1.0)
            {
              ...
              float curVal = texture2D(texture,curTexCoord).a;
