...
* @module app/scripts/fgx/shaders/matblur
*/
...
export default class MaterialBlur {
  ...
  */
  create(texture, texelSize, callbackMat) {
    ...
    this.m_strShaderVertex = new THREE.XHRLoader().load(BLUR_VERTEX_SHADER);
    this.m_strShaderFragment = new THREE.XHRLoader().load(BLUR_FRAGMENT_SHADER);

    Promise.all([this.m_strShaderVertex, this.m_strShaderFragment]).then(() => {
