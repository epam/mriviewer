...
/*
    ...
    this.rendererAO = new THREE.WebGLRenderer();
    this.rendererAO.setSize(this.xTex, this.yTex);
    const geometryAO = new THREE.PlaneGeometry(1.0, 1.0);
    const matAO = new MaterialAO();
    
    const VAL_4 = 4;
    this.vectors = new Uint8Array(VAL_4 * 16);
    for (let pix = 0; pix < 16; pix++) {
        this.vectors[pix * 4 + 0] = 128;
        this.vectors[pix * 4 + 1] = 128;
        this.vectors[pix * 4 + 1 + 1] = 128;
        this.vectors[pix * 4 + 1 + 1 + 1] = 0;
    }
    this.vectorsTex = new THREE.DataTexture(this.vectors, 16, 1, THREE.RGBAFormat);
    ...
    this.rendererAO.render(this.sceneAO, this.cameraOrtho, this.bufferTexture);
    const gl = this.rendererAO.getContext();
    const tmpAO = new Uint8Array(VAL_4 * this.xTex * this.yTex);
    gl.readPixels(0, 0, this.xTex, this.yTex, gl.RGBA, gl.UNSIGNED_BYTE, tmpAO);
