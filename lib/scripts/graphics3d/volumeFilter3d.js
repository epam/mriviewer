...
export default class VolumeFilter3d {
  ...
  */
  ...
  createTransferFuncTexture() {
    ...
    for (let pix = 0; pix < this.numTfPixels; pix++) {
      // eslint-disable-next-line
      this.transferFuncRgba[pix * 4 + 0] = 0;
      // eslint-disable-next-line
      this.transferFuncRgba[pix * 4 + 1] = 0;
      // eslint-disable-next-line
      this.transferFuncRgba[pix * 4 + 2] = 0;
      // eslint-disable-next-line
      this.transferFuncRgba[pix * 4 + 3] = 255;
    }
  ...
  * @param ctrlPts Array of control points of type Vector3 where (x,y,z) = color value in range [0, 1]
  */
  setTransferFuncColors(ctrlPts) {
    this.transferFuncCtrlPtsRgb = ctrlPts;
  }
  /**
  * Creates transfer function color map
  * @param ctrlPts Array of Vector2 where (x,y) = x coordinate in [0, 1], alpha value in [0, 1]
  */
  updateTransferFuncTexture(ctrlPts) {
    for (let curPt = 0; curPt < ctrlPts.length - 1; curPt++) {
      const pixStart = ctrlPts[curPt].x * this.numTfPixels;
      const pixEnd = ctrlPts[curPt + 1].x * this.numTfPixels;
      for (let pix = pixStart; pix < pixEnd; pix++) {
        ...
        // eslint-disable-next-line
        this.transferFuncRgba[pix * 4 + 3] = ctrlPts[curPt + 1].y * lerpVal + (1.0 - lerpVal) * ctrlPts[curPt].y;
      }
    }
  ...
  */
  copyFrameToTexture() {
    ...
    gl.readPixels(0, 0, this.xTex, this.yTex, gl.RGBA, gl.UNSIGNED_BYTE, this.bufferRgba);
  }
