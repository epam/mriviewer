...
export default class VolumeFilter3d {
  ...
  */
  ...
  {
   var ix = Math.floor( x_ * this.xDim);
   var iy = Math.floor( y_ * this.yDim);
   var iz = Math.floor( z_ * this.zDim);
   console.log ("eras: " + ix + " " + iy + " " + iz);
  ...
  for (let yTile = 0; yTile < this.zDimSqrt; yTile++) {
      ...
      for (let xTile = 0; xTile < this.zDimSqrt; xTile++) {
		...
		const rxy = Math.round(Math.max(Math.sqrt(2 * size - 1) / 2, Math.sqrt(size*size - iR*iR))  * radiusRatio);
        console.log("RXY: " + rxy);		
        const zVolOff = zVol * this.xDim * this.yDim;
        for (let y = Math.max(0, iy-rxy); y < Math.min(this.yDim, iy + rxy + 1); y++) {
          const yVol = y;
          const yVolOff = yVol * this.xDim;
		  
		  const iRy = y-iy;
		  const iRx = Math.round(Math.sqrt(rxy*rxy - iRy*iRy));

          for (let x = Math.max(0, ix-iRx); x < Math.min(this.xDim, ix + iRx + 1); x++) {
			const SIGMA = 1.5;
            for(let i = -Math.min(2, x); i <= Math.min(2, this.xDim - 1 -  x); i++){
			  for(let j = -Math.min(2, y); j <= Math.min(2, this.yDim - 1 - y); j++){
			    const offDst = yTileOff + xTileOff + ((y + j) * this.xTex) + x + i;
                const offDst4 = offDst * FOUR;   
			  
			  const gauss = 1 - Math.exp( -(i*i + j*j) / (2 * SIGMA * SIGMA)) / ( 2 * Math.PI * SIGMA * SIGMA);
