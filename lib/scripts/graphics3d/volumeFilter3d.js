...
export default class VolumeFilter3d {
  ...
  */
  ...
  /**
   * Dospose of the rendering context
   */
  dispose() {
    this.sceneBlur = null;
    this.rendererBlur.dispose();
  }
  ...
  */
  ...
  erasePixels(x_, y_, z_, size, depth, vDir) {
    ...
    if (vDir) {
      ...
      let normFactor = 0;
      const VAL_2 = 2;
      for (let k = -Math.min(GAUSS_R, targetZ);
        k <= Math.min(GAUSS_R, this.zDim - 1 - targetZ); k++) {
        for (let j = -Math.min(GAUSS_R, targetY);
          j <= Math.min(GAUSS_R, this.yDim - 1 - targetY); j++) {
          for (let i = -Math.min(GAUSS_R, targetX);
            i <= Math.min(GAUSS_R, this.xDim - 1 - targetX); i++) {
            // handling voxel:
    ...
    this.lastRotationMatrix = rotationMatrix.clone();
    this.lastTarget = new THREE.Vector3(targetX, targetY, targetZ);
  ...
  undoLastErasing() {
    ...
    const rxy = Math.round(this.lastSize);
    for (let z = 0; z < this.lastDepth + 1; z++) {
      for (let y = -rxy; y < rxy + 1; y++) {
        const rx = Math.round(Math.sqrt(rxy * rxy - y * y));
        for (let x = -rx; x < rx + 1; x++) {
          const point = new THREE.Vector3(x, y, z);
          point.applyMatrix3(this.lastRotationMatrix);

          const mainX = this.lastTarget.x + Math.round(point.x);
          const mainY = this.lastTarget.y + Math.round(point.y);
          const mainZ = this.lastTarget.z + Math.round(point.z / radiusRatio);
