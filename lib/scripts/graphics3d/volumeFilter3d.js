...
export default class VolumeFilter3d {
  ...
  */
  updateVolumeTexture(blurSigma, contrast, brightness, saveFlag) {
    ...
    console.log(`updateVolumeTexture`);
  }
  /**
  * Filtering the source data and building the normals on the GPU
  * @param blurSigma Gauss sigma parameter
      ...
      for (let xTile = 0; xTile < this.zDimSqrt; xTile++) {
        ...
        for (let y = 0; y < this.yDim; y++) {
          ...
          for (let x = 0; x < this.xDim; x++) {
            ...
            this.bufferTextureCPU[4*offDst + 0] = 0;//255;
            this.bufferTextureCPU[4*offDst + 1] = 0;
            this.bufferTextureCPU[4*offDst + 2] = 255;
            this.bufferTextureCPU[4*offDst + 3] = valInt;
          }
        }
      }
    }
  }
setBufferRgbaFrom4Bytes_() {
    const OFF0 = 0;
    // const OFF1 = 1;
    // const OFF2 = 2;
    const OFF3 = 3;
      ...
      for (let xTile = 0; xTile < this.zDimSqrt; xTile++) {
        ...
        for (let y = 0; y < this.yDim; y++) {
          ...
          for (let x = 0; x < this.xDim; x++) {
            ...
            this.bufferR[offDst + OFF0] = valInt;
            this.bufferRoi[offDst + OFF0] = valRoi;
            this.bufferTextureCPU[4*offDst + 0] = 255;
            this.bufferTextureCPU[4*offDst + 1] = 0;
            this.bufferTextureCPU[4*offDst + 2] = 0;
            this.bufferTextureCPU[4*offDst + 3] = valInt;
          }
        }
      }
  ...
  erasePixels(x_, y_, z_, size) {
    ...
    const yTileT = Math.floor(targetZ / this.zDimSqrt);
    const xTileT = targetZ - this.zDimSqrt * yTileT;
    const yTileOffT = (yTileT * this.yDim) * this.xTex;
    const xTileOffT = xTileT * this.xDim;
    const offDstT = yTileOffT + (targetY * this.xTex) + xTileOffT + targetX;
    const targetInd = offDstT * VAL_FOUR;

    const VAL_NORMAL_OFFSET = 128;

    const normal = new THREE.Vector3(this.bufferTextureCPU[targetInd + OFF0] - VAL_NORMAL_OFFSET,
      this.bufferTextureCPU[targetInd + OFF1] - VAL_NORMAL_OFFSET,
      this.bufferTextureCPU[targetInd + OFF2] - VAL_NORMAL_OFFSET);
    normal.normalize();
    ...
    rotationMatrix.makeBasis(vT, vB, normal.multiplyScalar(-1));

    //copy nearby part of volume to temporary buffer
    for (let z = Math.max(0, targetZ - this.maskGaussingDimZhalf);
      z < Math.min(this.zDim, targetZ + this.maskGaussingDimZhalf); z++) {
      const yTile = Math.floor(z / this.zDimSqrt);
      const xTile = z - this.zDimSqrt * yTile;
      const yTileOff = (yTile * this.yDim) * this.xTex;
      const xTileOff = xTile * this.xDim;
      const zBuffer = z - (targetZ - this.maskGaussingDimZhalf);
      const zBufferOff = zBuffer * this.maskGaussingDimXYhalf * VAL_2 * this.maskGaussingDimXYhalf * VAL_2;
      for (let y = Math.max(0, targetY - this.maskGaussingDimXYhalf);
        y < Math.min(this.yDim, targetY + this.maskGaussingDimXYhalf); y++) {
        const yBuffer = y - (targetY - this.maskGaussingDimXYhalf);
        for (let x = Math.max(0, targetX - this.maskGaussingDimXYhalf);
          x < Math.min(this.xDim, targetX + this.maskGaussingDimXYhalf); x++) {
          const xBuffer = x - (targetX - this.maskGaussingDimXYhalf);
          const offDst = yTileOff + (y * this.xTex) + xTileOff + x;
          this.maskGaussingTempBuf[zBufferOff + (this.maskGaussingDimXYhalf * VAL_2) * yBuffer + xBuffer]
            = this.bufferMask[offDst];
        }//x-coord end
      }//y-coord end
    }// z-coord end

    //erase in temp buffer pixels inside the required area
    const radiusRatio = this.xDim / this.zDim;
    const depth = 10;
    ////////////////////////////////////
    const rxy = Math.round(size  * radiusRatio);
    for (let z = 0; z < depth * radiusRatio + 1; z++) {
      for (let y = -rxy; y < rxy + 1; y++) {
        const rx = Math.round(Math.sqrt(rxy * rxy - y * y));
        for (let x = -rx; x < rx + 1; x++) {
          const point = new THREE.Vector3(x, y, z);
          point.applyMatrix4(rotationMatrix);
          const curX = Math.round(this.maskGaussingDimXYhalf + point.x);
          const curY = Math.round(this.maskGaussingDimXYhalf + point.y);
          const curZ = Math.round(this.maskGaussingDimZhalf + point.z / radiusRatio);
          const zBufferOff = curZ * this.maskGaussingDimXYhalf * VAL_2 * this.maskGaussingDimXYhalf * VAL_2;
          this.maskGaussingTempBuf[zBufferOff + (this.maskGaussingDimXYhalf * VAL_2) * curY + curX] = 0;
        }//x-coord end
      }//y-coord end
    }// z-coord end
    ...
    const rxyG = Math.round((size + GAUSS_R)  * radiusRatio);
    // copy data back to main data container
    for (let z = -GAUSS_R; z < (depth + GAUSS_R)  * radiusRatio + 1; z++) {
      for (let y = -rxyG; y < rxyG + 1; y++) {
        const rx = Math.round(Math.sqrt(rxyG * rxyG - y * y));
        for (let x = -rx; x < rx + 1; x++) {
          const point = new THREE.Vector3(x, y, z);
          point.applyMatrix4(rotationMatrix);
          ...
          const mainX = curX + targetX - this.maskGaussingDimXYhalf;
          const mainY = curY + targetY - this.maskGaussingDimXYhalf;
          const mainZ = curZ + targetZ - this.maskGaussingDimZhalf;
          ...
          let normFactor = 0;
          let value = 0;
          for (let k = -Math.min(GAUSS_R, curZ);
            k <= Math.min(GAUSS_R, VAL_2 * this.maskGaussingDimZhalf - 1 - curZ); k++) {
            for (let j = -Math.min(GAUSS_R, curY);
              j <= Math.min(GAUSS_R, VAL_2 * this.maskGaussingDimXYhalf - 1 - curY); j++) {
              for (let i = -Math.min(GAUSS_R, curX);
                i <= Math.min(GAUSS_R, VAL_2 * this.maskGaussingDimXYhalf - 1 - curX); i++) {
                // handling voxel:
                // (curZ + k; curY+ j; curX + i);
                const gX = curX + i;
                const gY = curY + j;
                const gZ = curZ + k;
                const zBufferOff = gZ * this.maskGaussingDimXYhalf * VAL_2 * this.maskGaussingDimXYhalf * VAL_2;

                const gauss = 1 - Math.exp(-(i * i + j * j + k * k) / (VAL_2 * SIGMA2));
                normFactor += gauss;
                const curVal = this.maskGaussingTempBuf[zBufferOff + (this.maskGaussingDimXYhalf * VAL_2) * gY + gX];
                value += curVal * gauss;
              }
            }
          }
          ...
          this.bufferMask[offDst] = value / normFactor;
        }//x-coord end
      }//y-coord end
    }// z-coord end
    ...
    //    let norm_factor = 0;
    //    let value = 0;
    //     for (let k = -Math.min(GAUSS_R, zBuffer);
    // k <= Math.min(GAUSS_R, VAL_2 * this.maskGaussingDimZhalf - 1 - zBuffer); k++) {
    //        for (let j = -Math.min(GAUSS_R, yBuffer);
    // j <= Math.min(GAUSS_R, VAL_2 * this.maskGaussingDimXYhalf - 1 - yBuffer); j++) {
    //          for (let i = -Math.min(GAUSS_R, xBuffer);
    // i <= Math.min(GAUSS_R, VAL_2 * this.maskGaussingDimXYhalf - 1 - xBuffer); i++) {
    //            // handling voxel:
    //            // (zBuffer + k; yBuffer + j; xBuffer + i);
    //            const curX = xBuffer + i;
    //            const curY = yBuffer + j;
    //            const curZ = zBuffer + k;
    //
    //            const zBufferOff = curZ * this.maskGaussingDimXYhalf * 2 * this.maskGaussingDimXYhalf * 2;
    //
    //            const gauss = 1 - Math.exp(-(i * i + j * j + k * k) / (VAL_2 * SIGMA2));
    //            norm_factor += gauss;
    //            const curVal =
    //              this.maskGaussingTempBuf[zBufferOff + (this.maskGaussingDimXYhalf * 2) * curY + curX];
    //            value += curVal * gauss;
    //
    //          }
    //        }
    ...
    //
    //      const offDst = yTileOff + (y * this.xTex) + xTileOff + x;
    //      this.bufferMask[offDst] = value / norm_factor;
    //    }//x-coord end
    //  }//y-coord end
    //}// z-coord end
    ...
    // z < Math.min(this.zDim, targetZ + this.maskGaussingDimZhalf); z++) {
      ...
      const xTileOff = xTile * this.xDim;
      const zBuffer = z - (targetZ - this.maskGaussingDimZhalf);
      const zBufferOff = zBuffer * this.maskGaussingDimXYhalf * 2 * this.maskGaussingDimXYhalf * 2;
      for (let y = Math.max(0, targetY - this.maskGaussingDimXYhalf);
      //y < Math.min(this.yDim, targetY + this.maskGaussingDimXYhalf); y++) {
        const yBuffer = y - (targetY - this.maskGaussingDimXYhalf);
        for (let x = Math.max(0, targetX - this.maskGaussingDimXYhalf);
        x < Math.min(this.xDim, targetX + this.maskGaussingDimXYhalf); x++) {
          ...
          const offDst = yTileOff + (y * this.xTex) + xTileOff + x;
          this.bufferMask[offDst] =
          this.maskGaussingTempBuf[zBufferOff + (this.maskGaussingDimXYhalf * 2) * yBuffer + xBuffer];
        }//x-coord end
      }//y-coord end
    }// z-coord end
*/
  ...
  */
  createUpdatableVolumeTex(engine2d, isRoiVolume, roiColors) {
    ...
    if (isRoiVolume) {
      this.bufferTextureCPU = new Uint8Array(4 * numPixelsBuffer);
    } else {
  ...
  */
  createUpdatableVolumeMask(engine2d) {
    ...
    const maskGaussingDimXYhalf = 64;
    const maskGaussingDimZhalf = Math.floor(maskGaussingDimXYhalf * this.zDim / this.xDim);
    this.maskGaussingDimXYhalf = maskGaussingDimXYhalf;
    this.maskGaussingDimZhalf = maskGaussingDimZhalf;
    const VAL_2 = 2;
    this.maskGaussingTempBuf = new Uint8Array(maskGaussingDimXYhalf *
      VAL_2 * maskGaussingDimXYhalf * VAL_2 * maskGaussingDimZhalf * VAL_2);
