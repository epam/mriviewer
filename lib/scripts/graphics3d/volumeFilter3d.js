...
export default class VolumeFilter3d {
  ...
  */
  ...
  getIntensity(pointX, pointY, pointZ) {
    const yTile = Math.floor(pointZ / this.zDimSqrt);
    const xTile = pointZ - this.zDimSqrt * yTile;
    const yTileOff = (yTile * this.yDim) * this.xTex;
    const xTileOff = xTile * this.xDim;

    const offDst = yTileOff + (pointY * this.xTex) + xTileOff + pointX;
  ...
  changeIntensity(targetX, targetY, targetZ) {
    const mainX = targetX;
    const mainY = targetY;
    const mainZ = targetZ;
  ...
  erasePixelsFloodFill(x_, y_, z_, startflag, mouseup) {
    ...
    while (stack.length !== 0) {
      ...
      let openClose = false;
      let xTmp = point.tX;
      while (this.getIntensity(xTmp, point.tY, point.tZ) >= intensityTarget) {
        xTmp--;
      }
      ...
      for (xTmp = leftBound; xTmp <= rightBound; xTmp++) {
        this.changeIntensity(xTmp, point.tY, point.tZ);

        if (openUp === false) {
          if (this.getIntensity(xTmp, point.tY + 1, point.tZ) >= intensityTarget) {
            stack.push({ 'tX': xTmp, 'tY': (point.tY + 1), 'tZ': point.tZ });
            openUp = true;
          }
        } else if (this.getIntensity(xTmp, point.tY + 1, point.tZ) < intensityTarget) {
          openUp = false;
        }
        ...
        if (openFar === false) {
          if (this.getIntensity(xTmp, point.tY, point.tZ + 1) >= intensityTarget) {
            stack.push({ 'tX':xTmp, 'tY':point.tY, 'tZ':(point.tZ + 1) });
            openFar = true;
          }
        } else if (this.getIntensity(xTmp, point.tY, point.tZ + 1) < intensityTarget) {
          openFar = false;
        }
