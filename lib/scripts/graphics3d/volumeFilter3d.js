...
export default class VolumeFilter3d {
  ...
  */
  ...
  erasePixels(x_, y_, z_, size, depth) {
    if (!this.isRoiVolume) {
      console.log('Is not ROI - no normals');
    }
    ...
    const targetInd = yTileOffT + (targetY * this.xTex) + xTileOffT + targetX;

    const nX = this.bufferTextureCPU[targetInd - 1] - this.bufferTextureCPU[targetInd + 1];
    const nY = this.bufferTextureCPU[targetInd - this.xTex] - this.bufferTextureCPU[targetInd + this.xTex];
    ...
    if (targetZ === 0) {
      ...
      const yTileTz1 = Math.floor((targetZ - 1) / this.zDimSqrt);
      const xTileTz1 = (targetZ - 1) - this.zDimSqrt * yTileT;
      const yTileOffTz1 = (yTileTz1 * this.yDim) * this.xTex;
      const xTileOffTz1 = xTileTz1 * this.xDim;
      indz1 = yTileOffTz1 + (targetY * this.xTex) + xTileOffTz1 + targetX;
    }

    if (targetZ === this.zDim - 1) {
      indz2 = targetInd;
    } else {
      const yTileTz2 = Math.floor((targetZ + 1) / this.zDimSqrt);
      const xTileTz2 = (targetZ + 1) - this.zDimSqrt * yTileT;
      const yTileOffTz2 = (yTileTz2 * this.yDim) * this.xTex;
      const xTileOffTz2 = xTileTz2 * this.xDim;
      indz2 = yTileOffTz2 + (targetY * this.xTex) + xTileOffTz2 + targetX;
    }

    const nZ = this.bufferTextureCPU[indz1] - this.bufferTextureCPU[indz2];
  ...
  //    const depthG = depth + boundarySize;
  //    const rxyG = rxy + boundarySize;
  //    // smooth boundary pixels ( boundary and +/- 1), write result to tempBuf
  //    let bufferIter = 0;
  //
  //    for (let z = -boundarySize; z < depthG  + 1; z++) {
  //      for (let y = -rxyG; y < rxyG + 1; y++) {
  //        const rx = Math.round(Math.sqrt(rxyG * rxyG - y * y));
  //        for (let x = -rx; x < rx + 1; x++) {
  //          if (x > -rx + boundarySize * VAL_2 && x < rx - boundarySize * VAL_2 && //
  //            y > -rxyG + boundarySize * VAL_2 && y < rxyG - boundarySize * VAL_2 && // handling cylinder walls
  //            z > boundarySize && z < depthG - boundarySize * VAL_2) { // but not ignoring cylinder foundation and cover
  //            continue;
  //          }
  //          const point = new THREE.Vector3(x, y, z);
  //          point.applyMatrix3(rotationMatrix);
  ...
  //          let normFactor = 0;
  //          let value = 0;
  //          for (let k = (-1) * Math.min(GAUSS_R, mainZ); k <= Math.min(GAUSS_R, this.zDim - 1 - mainZ); k++) {
  //            for (let j = -Math.min(GAUSS_R, mainY);
  //              j <= Math.min(GAUSS_R, this.yDim - 1 - mainY); j++) {
  //              for (let i = -Math.min(GAUSS_R, mainX);
  //                i <= Math.min(GAUSS_R, this.xDim - 1 - mainX); i++) {
  //                // handling voxel:
  //                // (mainX + i; ,mainY+ j; mainZ + k);
  //                const gX = mainX + i;
  //                const gY = mainY + j;
  //                const gZ = mainZ + k;
  //
  ...
  //                normFactor += gauss;
  //                value += this.bufferMask[offDst] * gauss;
  //              }
  //            }
  //          }// end gauss summation
  //          this.maskGaussingTempBuf[bufferIter] = value / normFactor;
  //          bufferIter++;
  //        }//x-coord end
  //      }//y-coord end
  //    }// z-coord end
  //
  //    console.log(`${bufferIter}`);
  //    if (bufferIter >= this.maskGaussingBufferSize) {
  //      console.log('Gaussing Buffer overflow');
  //    }
  //    // load smoothed data back to main texture
  //    bufferIter = 0;
  //
  //    for (let z = -boundarySize; z < depthG  + 1; z++) {
  //      for (let y = -rxyG; y < rxyG + 1; y++) {
  ...
  //
  //          const offDst = yTileOff + (mainY * this.xTex) + xTileOff + mainX;
  //          this.bufferMask[offDst] = this.maskGaussingTempBuf[bufferIter];
  //          bufferIter++;
  //        }//x-coord end
  //      }//y-coord end
  //    }// z-coord end
