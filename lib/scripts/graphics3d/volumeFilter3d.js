...
export default class VolumeFilter3d {
 ...
 //   this.texRoiId = null;
    ...
    matBlur.create(this.origVolumeTex, this.RoiVolumeTex, texelSize, this.texRoiId, (mat) => {
  ...
  */
  copyFrameToTexture() {
    ...
    const val_4 = 4;
    let tmpRgba = new Uint8Array(val_4 * this.xTex * this.yTex);
    const gl = this.rendererBlur.getContext();
    gl.readPixels(0, 0, this.xTex, this.yTex, gl.RGBA, gl.UNSIGNED_BYTE, tmpRgba);//this.bufferTextureCPU);
      ...
      for (let xTile = 0; xTile < this.zDimSqrt; xTile++) {
        ...
        for (let y = 0; y < this.yDim; y++) {
          ...
          for (let x = 0; x < this.xDim; x++) {
            ...
            this.bufferTextureCPU[offDst + OFF0] = valRoi;
          }
        }
      }
    }
...
*/
  ...
  createSelectedRoiMap() {
    ...
    for (let pix = 0; pix < this.numRois; pix++) {
      ...
      this.selectedROIs[pix * 4 + c1] = 0;
      this.selectedROIs[pix * 4 + c2] = 0;
      this.selectedROIs[pix * 4 + c3] = 0;
    }
