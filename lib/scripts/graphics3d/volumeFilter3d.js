...
export default class VolumeFilter3d {
  ...
  */
  ...
  setVolumeTexture(blurSigma) {
    if (!this.material) {
      console.log(`blure material null`);
      return;
    }
    if (this.isRoiVolume) {
      return;
        ...
        for (let y = 0; y < this.yDim; y++) {
          for (let x = 0; x < this.xDim; x++) {
            if (this.isRoiVolume) {
              ...
              const indxR = (x + y * w  + start);
              //const indxR = VAL_4 * (x + y * this.xDim);
              //const t = 255.0 * indxR / (VAL_4 * this.xDim * this.yDim);
              this.bufferTextureCPU[indxL] = tmpRgba[indxR];//255.0 * k / this.zDim;//this.bufferR[indxR];//tmpRgba[indxR];
              this.bufferTextureCPU[indxL + VAL_1] = tmpRgba[indxR + VAL_1];//255.0 * k / this.zDim;//this.bufferR[indxR];//tmpRgba[indxR + VAL_1];
              this.bufferTextureCPU[indxL + VAL_2] = tmpRgba[indxR + VAL_2];//255.0 * k / this.zDim;//this.bufferR[indxR];//tmpRgba[indxR + VAL_2];
              this.bufferTextureCPU[indxL + VAL_3] = tmpRgba[indxR + VAL_3];//255.0 * k / this.zDim;//this.bufferR[indxR];//tmpRgba[indxR + VAL_3];
            } else {
      ...
      for (let xTile = 0; xTile < this.zDimSqrt; xTile++) {
        ...
        for (let y = 0; y < this.yDim; y++) {
          ...
          for (let x = 0; x < this.xDim; x++) {
            ...
            const offDst = yTileOff + xTileOff + (y * this.xTex) + x;
            this.bufferR[offDst + OFF0] = valInt;
            this.bufferTextureCPU[4*offDst + OFF0]= valInt;
            this.bufferTextureCPU[4*offDst + OFF1]= valInt;
            this.bufferTextureCPU[4*offDst + OFF2]= valInt;
            this.bufferTextureCPU[4*offDst + OFF3]= valInt;//255.0 * zVol * (x + y) / ( this.zDimSqrt * this.zDimSqrt * (this.xDim + this.yDim));
