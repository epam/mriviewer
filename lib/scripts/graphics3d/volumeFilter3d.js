...
export default class VolumeFilter3d {
  ...
  */
  setTransferFuncColors(ctrlPtsColorsHex) {
    ...
    for (let i = 0; pix < ctrlPtsColorsHex.length; i++) {
  ...
  /**
  * Creates transfer function color map
  * @param ctrlPts Array of Vector2 where (x,y) = x coordinate in [0, 1], alpha value in [0, 1]//intensity [0,255] opacity [0,1]
  */
  updateTransferFuncTexture(intensities, opacities) {
    for (let curPt = 0; curPt < intensities.length - 1; curPt++) {
      const pixStart = intensities[curPt].x;
      const pixEnd = intensities[curPt + 1].x;
