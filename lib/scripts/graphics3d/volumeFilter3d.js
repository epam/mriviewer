...
export default class VolumeFilter3d {
  ...
  getIntensity(x, y, z) {
    ...
    const offDst = yTileOff + (mainY * this.xTex) + xTileOff + mainX;
    const intensityPoint = this.bufferTextureCPU[offDst];
    return intensityPoint;
  }

  changeIntensity(x, y, z) {
    const mainX = x;
    const mainY = y;
    const mainZ = z;
 ...
 /* erasePixelsFloodFill(x_, y_, z_, startflag, mouseup) {
    ...
    const intensityPoint = this.getIntensity(targetX, targetY, targetZ);
    ...
    while (stack.length !== 0) {
      ...
      let openFar = false;
      let openClose = false;
      while (this.getIntensity(point.x - 1, point.y, point.z) === intensityTarget) {
        point.x -= 1;
        while (this.getIntensity(point.x + 1, point.y, point.z) === intensityTarget) {
          this.changeIntensity(point.x, point.y, point.z);
          if (openUp === false) {
            if (this.getIntensity(point.x, point.y + 1, point.z) === intensityTarget) {
              stack.push({ 'tX':point.tX, 'tY':(point.tY + 1), 'tZ':point.tZ });
              openUp = true;
            }
          } else if (this.getIntensity(point.x, point.y + 1, point.z) !== intensityTarget) {
            openUp = false;
          }
          if (openDown === false) {
            if (this.getIntensity(point.x, point.y - 1, point.z) === intensityTarget) {
          ...
          if (openFar === false) {
            if (this.getIntensity(point.x, point.y, point.z + 1) === intensityTarget) {
              stack.push({ 'tX':point.tX, 'tY':point.tY, 'tZ':(point.tZ + 1) });
              openFar = true;
            }
          } else if (this.getIntensity(point.x, point.y, point.z + 1) !== intensityTarget) {
            openFar = false;
          }
          if (openClose === false) {
            if (this.getIntensity(point.x, point.y, point.z - 1) === intensityTarget) {
