...
export default class Graphics3d {
  ...
  */
  addSphereToSphereScene() {
    ...
    this.geometrySphere.addAttribute( 'position', new THREE.BufferAttribute( posAttribute, 3 ) );
    this.geometrySphere.addAttribute( 'normal', new THREE.BufferAttribute( normalsAttribute, 3 ) );
    this.geometrySphere.setIndex(new THREE.BufferAttribute(indAttribute, 1));
	
	
    //this.geometrySphere.fromGeometry(geo);
    // this.geometrySphere = geo;

    // const nv = this.geometrySphere.getAttribute('position').length;
    // console.log(`geometrySphere. numVertices = ${nv}`);

    this.computeGeometrySphereUVs(this.nonEmptyBoxMin, this.nonEmptyBoxMax);
  ...
  */
  ...
  computeGeometrySphereUVs(nonEmptyBoxMin, nonEmptyBoxMax) {
    this.geometrySphere.computeBoundingBox();
    ...
    if (NEED_SPHERE_UVW) {
      ...
      for (let i = 0; i < this.geometrySphere.getAttribute('position').count; i++) {
        ...
        // eslint-disable-next-line
        uvw[i * VAL_3 + 0] = -vx;// / this.vBoxVirt.x; //-(vx + this.geo_offset1.x) * this.geo_scale.x  + this.geo_offset2.x;
        // eslint-disable-next-line
        uvw[i * VAL_3 + 1] = +vy;// / this.vBoxVirt.y; //(vy + this.geo_offset1.y) * this.geo_scale.y  + this.geo_offset2.y;
        // eslint-disable-next-line
        uvw[i * VAL_3 + 2] = +vz;// / this.vBoxVirt.z; //(vz + this.geo_offset1.z) * this.geo_scale.z  + this.geo_offset2.z;
      }
      ...
      //this.geometryWireFrameSphere.addAttribute('uvw', new THREE.BufferAttribute(uvw, VAL_3));
      //this.geometryWireFrameSphere.getAttribute('uvw').needsUpdate = true;
    }
  ...
  render() {
    ...
    if (!matReady) {
      ...
      if (this.renderScene === SCENE_TYPE_RAYCAST) {
        ...
        // Try to render backface colors with frontface geometry
        // Take front face pass as clipplane + fronfaces with screenTex material
        this.renderer.setRenderTarget(this.bfTexture);
        this.renderer.clear();
        this.renderer.state.buffers.depth.setClear(1);
        this.matScreenTex.side = THREE.DoubleSide;
        this.matScreenTex.depthTest = false;
        this.matScreenTex.depthWrite = false,
        this.matScreenTex.needsUpdate = true;
        this.sceneClipPlane.overrideMaterial = this.matScreenTex;
        this.renderer.render(this.sceneClipPlane, this.camera, this.bfTexture);
        this.sceneClipPlane.overrideMaterial = null;
        this.matScreenTex.side = THREE.FrontSide;
        this.matScreenTex.depthTest = true;
        this.matScreenTex.depthWrite = true,
        this.matScreenTex.needsUpdate = true;
        this.scene.overrideMaterial = this.matScreenTex;
          // this.renderer.render(this.scene, this.camera, this.bfTexture);
        this.renderer.render(this.scene, this.camera, this.bfTexture);
        // Back faces done
        this.renderer.setRenderTarget(this.ffTexture);
    ...
    if (debug) {
      if (keyCode === KEY_CODE_F) {
        ...
        if (this.renderScene === SCENE_TYPE_RAYCAST) {
          ...
          //this.orbitControl.setMesh(this.meshSphere);
          //this.orbitControl.setWireMesh(this.meshSphereWireFrame);
          ...
          //this.orbitControl.setMesh(this.mesh);
          //this.orbitControl.setWireMesh(null);
          this.orbitControl.setEraserMode(false);
        }
        //this.orbitControl.setMesh(this.meshSphere);
        //this.orbitControl.setWireMesh(this.mesh);
      } // if key code
      ...
      if (keyCode === KEY_CODE_PLUS) {
        this.geometrySphere.translate(-this.sculptingSphereCenter.x, -this.sculptingSphereCenter.y, -this.sculptingSphereCenter.z);
        //this.sculptingSphereSize *= ONE + scaleVal;
        //this.sculptingSphereCenter.multiplyScalar(ONE + scaleVal);
        this.geometrySphere.scale(ONE + scaleVal, ONE + scaleVal, ONE + scaleVal);
        this.geometrySphere.translate(this.sculptingSphereCenter.x, this.sculptingSphereCenter.y, this.sculptingSphereCenter.z);
        //this.geometryWireFrameSphere.scale(ONE + scaleVal, ONE + scaleVal, ONE + scaleVal);
        this.computeGeometrySphereUVs(this.nonEmptyBoxMin, this.nonEmptyBoxMax);
      }
