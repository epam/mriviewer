...
export default class Graphics3d {
  ...
  */
  ...
  createTetraGeometry() {
    ...
    const material = new THREE.MeshBasicMaterial({ color: 0xFF0000FF, depthTest: false });
...
//      const z = Math.random() * 2 - 1;
  ...
  render() {
    ...
    if (!matReady) {
      ...
      // render tetra stuff
      // this.renderer.render(this.sceneTetra, this.camera);
      this.renderCounter++;
    }
  }
  ...
  onMouseDown(xx, yy, ctrlKey) {
    ...
    if (this.isEraseMode && ctrlKey) {
      ...
      if (this.eraserNormalMode) {
        this.volumeUpdater.erasePixels(vX, vY, vZ, this.eraserRadius, this.eraserDepth, false, true, false);
      } else {
        this.volumeUpdater.erasePixels(vX, vY, vZ, this.eraserRadius, this.eraserDepth, vDir, true, false);
      }
    }
  }
  ...
  onMouseMove(xx, yy, ctrlKey) {
    ...
    if (this.isEraseMode && this.eraserMouseDown && ctrlKey) {
      ...
      vZ = vZ / length * this.bufferRenderToTextureCPU[bigCellInd + OFF3] + COORD_SHIFT +
        this.bufferFFTextureCPU[cellInd + OFF2];
      if (this.eraserNormalMode) {
        this.volumeUpdater.erasePixels(vX, vY, vZ, this.eraserRadius, this.eraserDepth, false, false, this.mouseupflag);
      } else {
        this.volumeUpdater.erasePixels(vX, vY, vZ, this.eraserRadius, this.eraserDepth, vDir, false, this.mouseupflag);
      }
