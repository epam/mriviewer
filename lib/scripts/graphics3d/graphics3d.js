...
export default class Graphics3d {
  ...
  */
  addSphereToSphereSceneWireFrame() {
    ...
    const vRadius = new THREE.Vector3(1.0, 1.0, 1.0);
    ...
    this.meshSphereWireFrame = new THREE.Mesh(bGeo, matThreeJs);
  ...
  /**
  * Special scene with sphere: add new generated Three js geometry (sphere)
  * geoThreeJs: THREE.Geometry
  */
  addSphereToSphereScene(geoThreeJs) {
    // remove all old meshes from scene
    const objs = this.sceneSphere.children;
    const numOldMeshes = objs.length;
    for (let j = numOldMeshes - 1; j >= 0; j--) {
      const obj = objs[j];
      this.sceneSphere.remove(obj);
    }

    this.geometrySphere = new THREE.BufferGeometry();
    this.geometrySphere.fromGeometry(geoThreeJs);
    this.computeGeometrySphereUVs(this.nonEmptyBoxMin, this.nonEmptyBoxMax);

    // create mesh on provided geo and add to scene
    const matThreeJs = new THREE.MeshBasicMaterial({
      color: 0xFFFF0077, depthTest: false,
      wireframe: true, side: THREE.DoubleSide
    });
    this.meshSphere = new THREE.Mesh(this.geometrySphere, matThreeJs);
    this.sceneSphere.add(this.meshSphere);
    // console.log('addSphereToSphereScene() : added');
  }

  createAndAddSphereToSphereScene() {
    const gen = new TetrahedronGenerator();
    const MAG_SCALE = 0.4;
    const vRadius = new THREE.Vector3(MAG_SCALE, MAG_SCALE, MAG_SCALE);
    ...
    this.addSphereToSphereScene(geo);
  ...
  */
  ...
  updateClipPlaneGeometry() {
    ...
    for (let i = 0; i < this.planeGeometry.getAttribute('position').count; i++) {
      ...
      */
      // eslint-disable-next-line
      uvw[i * VAL_3 + OFF_0] = -v.x;
      // eslint-disable-next-line
      uvw[i * VAL_3 + OFF_1] = v.y;
      // eslint-disable-next-line
      uvw[i * VAL_3 + OFF_2] = v.z;
    }
  ...
  callbackCreateCubeVolumeBF(window, box, nonEmptyBoxMin, nonEmptyBoxMax, isRoiVolume = false) {
    ...
    this.renderScene = SCENE_TYPE_RAYCAST;
    if (!this.meshSphere) {
      this.createAndAddSphereToSphereScene();
    }
...
//      const z = Math.random() * 2 - 1;
  ...
  */
  ...
  resetEraser() {
    ...
    this.render();
    console.log('Erase mode: resetBufferTextureCPU');
  }
  ...
  onMouseDown(xx, yy, ctrlKey) {
    ...
    if (this.isEraseMode && ctrlKey) {
      ...
      if (this.eraserNormalMode) {
        ...
        this.volumeUpdater.erasePixelsFloodFill(vX, vY, vZ, true, false);
      } else {
        this.volumeUpdater.erasePixels(vX, vY, vZ, this.eraserRadius, this.eraserDepth, vDir,
