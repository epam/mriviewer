...
export default class Graphics3d {
  ...
  */
  ...
  updateClipPlaneGeometry() {
    ...
    for (let i = 0; i < this.planeGeometry.getAttribute('position').count; i++) {
      ...
      v.z = this.planeGeometry.getAttribute('position').getZ(i);
  ...
  */
  callbackCreateCubeVolumeBF(window, box, nonEmptyBoxMin, nonEmptyBoxMax, isRoiVolume = false) {
    ...
    const zDimSqrt = this.volumeUpdater.getTextureTileFactor();
  ...
  */
  updateCutPlanes() {
    ...
    console.log(`Center point : ${this.planeCenterPt.x} ${this.planeCenterPt.y} ${this.planeCenterPt.z}`);
  ...
  */
  ...
  onMouseDown(xx, yy, ctrlKey) {
    ...
    if (this.isEraseMode && ctrlKey) {
      ...
      //console.log("LENGTHS: " + this.bufferBFTextureCPU.length + " " +
      //this.bufferFFTextureCPU.length + " " + this.bufferRenderToTextureCPU.length);
      ...
      const TWO = 2;
      if (this.bufferRenderToTextureCPU[bigCellInd + OFF3] === TWO) {
        return;
      }
      ...
      vX = vX / length * this.bufferRenderToTextureCPU[bigCellInd + OFF3] + 0.5 +
      this.bufferFFTextureCPU[cellInd + OFF0];
      vY = vY / length * this.bufferRenderToTextureCPU[bigCellInd + OFF3] + 0.5 +
      this.bufferFFTextureCPU[cellInd + OFF1];
      vZ = vZ / length * this.bufferRenderToTextureCPU[bigCellInd + OFF3] + 0.5 +
      this.bufferFFTextureCPU[cellInd + OFF2];
      //console.log("Vec: " + vX + "; " + vY + "; " + vZ);

      const FIVE5 = 5;
      this.volumeUpdater.erasePixels(vX, vY, vZ, FIVE5);
    }
  }
