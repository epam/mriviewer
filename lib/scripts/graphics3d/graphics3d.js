...
export default class Graphics3d {
  ...
  */
  callbackCreateCubeVolumeBF(window, box, nonEmptyBoxMin, nonEmptyBoxMax, isRoiVolume = false) {
    ...
    matRenderToTextureThreeGS.create(this.texTF, this.volTexture, this.volTextureMask, this.bfTexture, this.ffTexture,
      (mat) => {
    ...
    matSkullThreeGS.create(this.texTF, this.volTexture, this.volTextureMask, this.bfTexture, this.ffTexture,
  ...
  */
  ...
  onKeyDown(keyCode, debug) {
    ...
    const COL_LO = 100;
    const COL_HI = 240;
    const buff = new Uint8Array(NUM_COLORS);
    if (keyCode === KEY_CODE_E) {
      for (let pix = 0; pix < NUM_COLORS; pix++) {
        buff[pix] = false;
      }
      for (let pix = COL_LO; pix < COL_HI; pix++) {
        buff[pix] = true;
      }
      this.volumeUpdater.updateSelectedRoiMap(buff);
    } // if pressed 'E' key
  }
