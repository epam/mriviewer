...
export default class Graphics3d {
  ...
  */
  callbackCreateCubeVolumeBF(window, box, nonEmptyBoxMin, nonEmptyBoxMax, isRoiVolume = false) {
    ...
    // display texture as debug window
    /*
    const pixelsSrc1Byte = this.engine2d.m_volumeData;
    const xDim = header.m_pixelWidth;
    const yDim = header.m_pixelHeight;
    const numPixels = xDim * yDim * zDim;
    this.volTexture = new Uint8Array(numPixels * 4);
    // copy 1 byte volume texture to 4 byte
    let i, j;
    for (i = 0, j = 0; i < numPixels; i++, j += 4) {
      this.volTexture[j + 0] = pixelsSrc1Byte[i];
      this.volTexture[j + 1] = 0;
      this.volTexture[j + 2] = 0;
      this.volTexture[j + 3] = 0;
    }
    // create temporary plane
    const bufPixels2d = new Uint8Array(xDim * yDim * 4);
    const X_SLICE = 0;
    const Y_SLICE = 1;
    const Z_SLICE = 2;

    VolumeTools.extract2dSliceFrom3dTexture(xDim, yDim, zDim, this.volTexture,
      Z_SLICE, Math.floor(zDim / 2), bufPixels2d);
