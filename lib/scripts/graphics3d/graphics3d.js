...
export default class Graphics3d {
  ...
  */
  callbackCreateCubeVolumeBF(window, box, nonEmptyBoxMin, nonEmptyBoxMax, isRoiVolume = false) {
    ...
    if (this.renderer.getContext().getExtension('OES_texture_float')) {
	  ...
	  this.bfTexture = new THREE.WebGLRenderTarget(this.windowWidth,
        this.windowHeight, {
    ...
    matRenderToTextureThreeGS.create(this.texTF, this.volTexture, this.bfTexture, this.ffTexture, (mat) => {
    ...
    matSkullThreeGS.create(this.texTF, this.volTexture, this.bfTexture, this.ffTexture, this.renderToTexture, (mat) => {
      mat.uniforms.t_function1min.value =
        new THREE.Vector4(VOLUME_COLOR1_MIN_R, VOLUME_COLOR1_MIN_G, VOLUME_COLOR1_MIN_B,
          this.curFileDataType.thresholdTissue1);
      mat.uniforms.t_function1max.value =
  ...
  */
  ...
  var y =  Math.round(yy);
	...
	var cellInd = (y * this.windowWidth + x) * GPU_CELL_SIZE;
	console.log("LENGTHS: " + this.bufferBFTextureCPU.length + " " + this.bufferFFTextureCPU.length + " " + this.bufferRenderToTextureCPU.length);
	console.log("BackFace Vec X:" + this.bufferBFTextureCPU[cellInd + OFF0]);
	console.log("BackFace Vec Y:" + this.bufferBFTextureCPU[cellInd + OFF1]);
	console.log("BackFace Vec Z:" + this.bufferBFTextureCPU[cellInd + OFF2]);
	//console.log("BackFace Vec A:" + this.bufferBFTextureCPU[cellInd + OFF3]);
	
	 console.log("FrontFace Vec X:" + this.bufferFFTextureCPU[cellInd + OFF0]);
	 console.log("FrontFace Vec Y:" + this.bufferFFTextureCPU[cellInd + OFF1]);
	 console.log("FrontFace Vec Z:" + this.bufferFFTextureCPU[cellInd + OFF2]);
	 //console.log("FrontFace Vec A:" + this.bufferFFTextureCPU[cellInd + OFF3]);
	 
	 var bigCellInd = (Math.floor(y / 3) * this.xSmallTexSize + Math.floor(x / 3)) * GPU_CELL_SIZE;
	 //console.log("BSI: " + bigCellInd); 
  	 //console.log("Dist: " + this.bufferRenderToTextureCPU[bigCellInd + OFF0]);
	 //console.log("Dist: " + this.bufferRenderToTextureCPU[bigCellInd + OFF1]);
	 //console.log("Dist: " + this.bufferRenderToTextureCPU[bigCellInd + OFF2]);
	 console.log("Dist: " + this.bufferRenderToTextureCPU[bigCellInd + OFF3]);
	 
	 if(this.bufferRenderToTextureCPU[bigCellInd + OFF3] == 2)
	   return;
	 
	 var vX = this.bufferBFTextureCPU[cellInd + OFF0] - this.bufferFFTextureCPU[cellInd + OFF0];
	 var vY = this.bufferBFTextureCPU[cellInd + OFF1] - this.bufferFFTextureCPU[cellInd + OFF1];
	 var vZ = this.bufferBFTextureCPU[cellInd + OFF2] - this.bufferFFTextureCPU[cellInd + OFF2];
	 
	 console.log("Vec: " + vX + "; " + vY + "; " + vZ);
	 var length = Math.sqrt(vX * vX + vY * vY + vZ * vZ);
     vX = vX / length * this.bufferRenderToTextureCPU[bigCellInd + OFF3] + 0.5 + this.bufferFFTextureCPU[cellInd + OFF0]; 
	 vY = vY / length * this.bufferRenderToTextureCPU[bigCellInd + OFF3] + 0.5 + this.bufferFFTextureCPU[cellInd + OFF1]; 
	 vZ = vZ / length * this.bufferRenderToTextureCPU[bigCellInd + OFF3] + 0.5 + this.bufferFFTextureCPU[cellInd + OFF2]; 
	 
	 console.log("Vec: " + vX + "; " + vY + "; " + vZ);
	 
	 this.volumeUpdater.erasePixels(vX, vY, vZ, 5);
     this.volTexture.needsUpdate = true;
	 }
  }
