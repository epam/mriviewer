  ...
  onKeyDown(keyCode, debug) {
    ...
    if (debug) {
      if (keyCode === KEY_CODE_C) {
        let volTextureSrc = this.m_volumeData;
        let xDim = this.m_volumeHeader.m_pixelWidth;
        let yDim = this.m_volumeHeader.m_pixelHeight;
        let zDim = this.m_volumeHeader.m_pixelDepth;
        const TWICE = 2;
        if (zDim < xDim / TWICE) {
          const xNew = xDim >> 1;
          const yNew = yDim >> 1;
        ...
        const NEED_LOG = true;
        const NEED_CREATE_MASK = false;
        const actVolume = new ActiveVolume();
        actVolume.skullRemove(xDim, yDim, zDim, volTextureSrc, volTextureDst, NEED_CREATE_MASK, NEED_LOG);
      }
