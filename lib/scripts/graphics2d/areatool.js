...
/**
 * 2d area tool
 * @module lib/scripts/graphics2d/areatool
 */
...
export default class AreaTool {
  ...
  constructor(scene, lineWidth) {
    ...
    /** @property {float} last_length - number of the closing vertex of the last polygon */
    this.last_length = 0;
    ...
    if (this.m_runningState) {
      ...
      if (length > this.last_length + MINIMUM_COUNT_OF_VERTEX) {
        if (this.connect_line === true) {
          ...
          this.m_vertexes.pop();
          length = this.m_vertexes.length;
          this.m_area = this.calculateArea(length);
          this.drawLine(this.m_vertexes[this.last_length], this.m_vertexes[length - 1],
