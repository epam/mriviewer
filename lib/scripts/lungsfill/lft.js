  ...
  */
  ...
  run() {
    ...
    if (!this.VESSEL) {
      ...
      fillTool.floodFill3dThreshold(this.m_xDim, this.m_yDim, this.m_zDim, this.m_volTexMask1, vSeed, 5);
      
      for (x = 0; x < xyzDim; x++) {
        let val = 0.5 * this.m_volTexMask[x];//0;
        if (this.m_volTexMask2[x] - this.m_volTexMask[x] === VIS || this.m_volTexMask1[x] === VIS) {
