...
export default class Med3Web {
  ...
  */
  performSkullRemove() {
    ...
    const resSkullRem =
      actVolume.skullRemove(xDim, yDim, zDim, volTextureSrc, volTextureDst, CREATE_TYPE, NEED_LOG, (ratio) => {
        if (this.m_menu) {
          this.m_menu.updateProgressBar(ratio);
        } else {
          console.log(`skullRemove: ${ratio}`);
        }
      });
    if (resSkullRem !== 1) {
