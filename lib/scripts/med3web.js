  ...
  */
  constructor(opts) {
    if (typeof opts === 'undefined') {
      consoloe.log('Med3Web constructor has no params');
    }
    ...
    /** Is in 3d mode (default) or 2d mode */
    this.m_renderMode = RenderMode.RENDER_MODE_3D;
  ...
  _callbackLoadLocalDicomFolder(files) {
    ...
    loaderDicom.readFiles(files, (status, header, dataSize, dataArray) => {
      if (status === LoadResult.SUCCESS) {
        ...
        this._dicomOnFail(files[0].name, status);
      }
    }, (ratioLoaded) => {
      console.log(`Loading ratio = ${ratioLoaded}`);
      if (this.m_menu) {
