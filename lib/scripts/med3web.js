...
export default class Med3Web {
  ...
  */
  ...
  evolveSetBarrier(valBarrierUI) {
    // 0.46 is mapped into 0.2
    const VAL_TARGET = 0.2;
    const VAL_SRC = 0.46;
  ...
  */
  performSphereEvolve() {
    ...
    this.activeVolume.sphereEvolve(xDim, yDim, zDim, volTexSrc, volTexDst,
      geoSphere, geoWireSphere, this.m_evolveBarrier);
    eng3d.computeGeometrySphereUVs(eng3d.nonEmptyBoxMin, eng3d.nonEmptyBoxMax);
    // eng3d.render();
    // console.log(`test... ${volTexDst[0]}`);
  }
  ...
  finalizeLoadedData(header, dataArrayBytes, volumeInfo, box, nonEmptyBoxMin, nonEmptyBoxMax, isRoiVolume) {
    ...
    if ((NEED_CHECK_POWER_OF_TWO) && (!isPowerOfTwoX || !isPowerOfTwoY)) {
      ...
      const xNewDim = VolumeTools.getGreatOrEqualPowerOfTwo(xDim);
      const yNewDim = VolumeTools.getGreatOrEqualPowerOfTwo(yDim);

      const dataArrayNew = VolumeTools.makeTextureSizePowerOfTwoXY(dataArrayBytes, xDim, yDim, zDim, xNewDim, yNewDim);
      const zNewDim = zDim;
